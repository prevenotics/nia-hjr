...
json_folder = r'/root/work/prevenotics/hjr/dataset/2.라벨링데이터_mat'
output_parent_folder = r'/root/work/prevenotics/hjr/dataset/2.라벨링데이터_mat_new'
    ...
    for root, dirs, files in os.walk(json_folder):
        # if "001.RGB" in root:  # 경로에 "001.RGB"가 포함되어 있는지 확인
            for json_filename in files:
                if json_filename.endswith('.mat'):
                    mat_path = os.path.join(root, json_filename)
                    ...
                    print(f"Currnet file is {mat_path}")
                    mat_data = io.loadmat(mat_path)
                    
                    data_array = mat_data['label']  # 'your_variable_name'은 MAT 파일에서 사용하는 변수 이름입니다.

                    # 값이 30인 요소를 255로 변경
                    data_array[data_array == 30] = 255
                    
                    mat_filename = os.path.splitext(json_filename)[0] + ".mat"                    
                    mat_path = os.path.join(tiff_folder, mat_filename)                    
                    io.savemat(mat_path, {'label' : data_array})
